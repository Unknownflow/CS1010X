...
def repetitions(s):
    ...
    while sub_str_len < length:
        if length % sub_str_len != 0:
            sub_str_len += 1
        else:
            is_repeated = True
            sub_str = s[:sub_str_len]
            for i in range(sub_str_len, length, sub_str_len):
                if s[i: i+sub_str_len] != sub_str:
                    is_repeated = False

            if is_repeated:
                return length // sub_str_len
            else:
                sub_str_len += 1
...
def test2a():
    ...
    answer1 = [['Christopher Nolan', 'Adventure|Drama|Sci-Fi', 'Matthew McConaughey', 'Interstellar', '8.6', '2014'],
               ['Alan Taylor', 'Action|Adventure|Sci-Fi',
                   'J.K. Simmons', 'Terminator Genisys', '8.6', '2015'],
               ['Francis Lawrence', 'Adventure|Sci-Fi', 'Jennifer Lawrence',
                'The Hunger Games: Mockingjay - Part 2', '8.6', '2015'],
               ['Duncan Jones', 'Action|Adventure|Fantasy',
                'Dominic Cooper', 'Warcraft', '8.6', '2016'],
...
def test2b():
    ...
    print(best_movies("IMDB_small.csv", "Action", 3)
          == ['Terminator Genisys', 'Warcraft'])
    print(best_movies("IMDB.csv", "Comedy", 5) == [
          'Up', 'Monsters vs. Aliens', 'Evan Almighty', 'Suicide Squad', 'Wild Wild West'])
    print(best_movies("IMDB.csv", "Action", 3) == [
...
def test3():
    ...
    print(s1.winner(5) == ['Joseph Schooling', 'Laszlo Cseh',
          'Tom Shields', 'Michael Phelps', 'Mehdy Metella'])
    print(s1.winner(6) == ['Joseph Schooling', 'Laszlo Cseh',
          'Tom Shields', 'Michael Phelps', 'Mehdy Metella'])
